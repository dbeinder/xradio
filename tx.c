		  ...
		  u8 use_bg_rate, bool *renew)
{
	...
	if (idx >= 0) {
		txrx_printk(XRADIO_DBG_MSG, "[TX policy] Used TX policy: %d\n",
					idx);
		tx_policy_dump(&wanted);
		tx_policy_dump(&cache->cache[idx].policy);
		*renew = false;
	} else {
		struct tx_policy_cache_entry *entry;
...
{
	if (hw_value <= WSM_TRANSMIT_RATE_11)
		return hw_value;
	if (hw_value <= WSM_TRANSMIT_RATE_54)
		return hw_value - 2;
...
rate_table_full:
		...
		txrx_printk(XRADIO_DBG_NIY,"[TX retries] Feedback: " \
